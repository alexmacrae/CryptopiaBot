...
class Cryptopia extends Exchange {
    ...
    public function activeOrders($symbol = "") {
        ...
        $orders = array();
        $price = array();  // sort by price < edit
        $timstamp = array();  // sort by timestamp
        if ($myOrders['Success'] == "true" && $myOrders['Error'] == "") {
            foreach ($myOrders['Data'] as $order) {
                ...
                $orders[] = ["id" => $order["OrderId"], "symbol" => $orderSymbol, "type" => $order["Type"], "price" => $order["Rate"],
                    "amount" => $order["Amount"], "total" => $order["Total"], "remaining" => $order["Remaining"], "timestamp" => $order["TimeStamp"]];
                if ($order["Type"] == "Sell") {
                    $price[] = 0 - $order['Rate'];  // lowest ask price if first
                } else {
                    $price[] = $order['Rate'];
                }
                $timstamp[] = $order["TimeStamp"];
            }
            if ($orders) // If there are any orders
                array_multisort($timstamp, SORT_DESC, $orders); // sort orders by price
        } else {
    ...
    public function tradeHistory($symbol = "") {
        if ($symbol == "") {
            $apiParams = array('TradePairId' => "", 'Count' => '5000');
        } else {
            $apiParams = array('TradePairId' => $this->getExchangeSymbol($symbol), 'Count' => '5000');
        }
        $myHistory = json_decode($this->apiCall("GetTradeHistory", $apiParams), true);
      
        $orders = array();
        $timstamp = array();  // sort by timestamp
        if ($myHistory['Success'] == "true" && $myHistory['Error'] == "") {
            foreach ($myHistory['Data'] as $order) {
                
                $orders[] = ["id" => $order["TradeId"], "symbol" => $order["Market"], "type" => $order["Type"], "price" => $order["Rate"],
                    "amount" => $order["Amount"], "total" => $order["Total"],"fee" => $order["Fee"], "timestamp" => $order["TimeStamp"]];
            ...
            throw new Exception("Can't get active orders, Error: " . $myHistory['Error']);
        }
        return $orders;
    }
    ...
    public function cancelAll() {
        if (!$this->activeOrders())
            return false;  // "No open orders to cancel.\n"
